export default {
  ...
  'resources.preheat.profile.maintenanceHint': 'Lifecycle is independent of connection health. Maintenance profiles are excluded from new download, sync, preheat, policy, and schedule selections.',
  'resources.preheat.profile.maintenanceAction': 'Enter Maintenance',
  'resources.preheat.profile.maintenanceActionHint': 'Stops new work from using this profile without deleting S3 data.',
  'resources.preheat.profile.restoreAction': 'Restore Use',
  'resources.preheat.profile.restoreActionHint': 'Makes the profile available for new work again without making it the default.',
  'resources.preheat.profile.maintenanceConfirm': 'Enter Maintenance Mode',
  'resources.preheat.profile.maintenanceContent': 'Put S3 profile "{name}" into maintenance? It will be excluded from download, sync, preheat, policy, and schedule selections. S3 data is not deleted, and existing tasks continue with their fixed configuration.',
  'resources.preheat.profile.restoreConfirm': 'Restore S3 Profile',
  'resources.preheat.profile.restoreContent': 'Restore use of S3 profile "{name}"? It will not become the default automatically.',
  'resources.preheat.profile.credentialUnavailableHint': 'When the connection is unavailable, update Access Key and Secret Key in the manual profile. This does not enter maintenance automatically.',
  'resources.preheat.profile.systemCredentialUnavailableHint': 'When the connection is unavailable, update system credentials through startup arguments and restart. This does not enter maintenance automatically.',
  'resources.preheat.profile.connectivityUnavailableHint': 'A connection failure can involve DNS, TLS, network, authentication, or the bucket. Open connection details to confirm the failed stage; then update credentials, TLS, or addressing in a manual profile. It does not enter maintenance automatically.',
  'resources.preheat.profile.systemConnectivityUnavailableHint': 'A connection failure can involve DNS, TLS, network, authentication, or the bucket. Open connection details to confirm the failed stage; update managed fields through startup arguments and restart, while editable runtime switches remain available here. It does not enter maintenance automatically.',
  ...
  'resources.preheat.policy.continuous': 'Continuous',
  'resources.preheat.policy.scheduled': 'Scheduled',
  ...
  'resources.preheat.schedule.actionContent': 'Perform this action on scheduled strategy "{name}"?',
  ...
  'resources.storage.title': 'Model Storage', 'resources.storage.description': 'Verified node models and the shared S3 model library are reused by sync, regular downloads, and preheat.', 'resources.storage.nodeModels': 'Node Models', 'resources.storage.library': 'S3 Model Library', 'resources.storage.syncTasks': 'Sync Tasks', 'resources.storage.preheatTasks': 'Preheat Tasks', 'resources.storage.policies': 'Distribution Policies', 'resources.storage.profiles': 'S3 Profiles', 'resources.storage.artifacts': 'Synced Models', 'resources.storage.connectivity': 'Worker Connectivity', 'resources.storage.model': 'Model', 'resources.storage.modelSource': 'Model Source', 'resources.storage.sourceWorker': 'Source Worker', 'resources.storage.version': 'Version', 'resources.storage.fileCount': 'Files', 'resources.storage.capacity': 'Capacity', 'resources.storage.targetProfile': 'Target S3 Profile', 'resources.storage.sync': 'Sync to S3 Model Library', 'resources.storage.sync.confirmTitle': 'Sync to S3 Model Library', 'resources.storage.sync.submit': 'Start Sync', 'resources.storage.refresh': 'Refresh Inventory', 'resources.storage.refreshConfirm': 'Refresh S3 Inventory', 'resources.storage.refreshContent': 'Scan models with valid manifests in the selected S3 profile?', 'resources.storage.cancelSync': 'Cancel Sync', 'resources.storage.cancelSyncConfirm': 'Cancel Sync Task', 'resources.storage.cancelSyncContent': 'Cancel or delete sync task #{id}?', 'resources.storage.systemProfile': 'System Configuration', 'resources.storage.setDefault': 'Set Default', 'resources.storage.setDefaultConfirm': 'Switch Default S3 Profile', 'resources.storage.setDefaultContent': 'Only new downloads and tasks are affected; running tasks keep their current profile.', 'resources.storage.checkWorkers': 'Check Workers', 'resources.storage.connectionScope': 'Test Connection checks Server to S3; Check Workers checks Worker to S3.', 'resources.storage.testConnection': 'Test Connection', 'resources.storage.testResult': 'Server Connection Test', 'resources.storage.encryptionUnavailable': 'Server credential encryption is unavailable. Contact an administrator to check the data directory or key configuration.', 'resources.storage.endpointTlsMismatch': 'The endpoint protocol and TLS switch do not match.', 'resources.storage.sourceFallback': 'Allow download from model source when missing', 'resources.storage.sourceFallbackHint': 'This switch affects regular model downloads only when this profile is the default.', 'resources.storage.testCredentialsRequired': 'Re-enter Access Key and Secret Key before testing.', 'resources.storage.refreshSubmitted': 'Inventory refresh was submitted and is waiting for scan results.', 'resources.storage.checkWorkersConfirm': 'Check Worker Connectivity', 'resources.storage.checkWorkersContent': 'Check Worker connectivity to S3 profile “{name}”?', 'resources.storage.transferMethod': 'Transfer Method', 'resources.storage.syncTaskDetail': 'Sync Task Detail', 'resources.storage.refreshCompleted': 'Inventory refresh completed.', 'resources.storage.refreshFailed': 'Inventory refresh failed.', 'resources.storage.transfer.current_node': 'Current node ({worker})', 'resources.storage.transfer.peer_via_s3': 'Another node ({worker}) via S3 model library {profile}', 'resources.storage.transfer.s3': 'S3 model library ({profile})', 'resources.storage.transfer.modelscope': 'Downloaded from ModelScope', 'resources.storage.transfer.huggingface': 'Downloaded from Hugging Face', 'resources.storage.transfer.unknown': 'Unknown transfer method',
  'resources.storage.syncTask.errorCode': 'Failure Reason',
  'resources.storage.syncTask.error.worker_execution_failed': 'Worker failed to execute the sync task',
  'resources.storage.syncTask.error.manifest_invalid': 'The model manifest is invalid',
  'resources.storage.syncTask.error.s3_object_conflict': 'The model object conflicts with an existing S3 object',
  'resources.storage.tlsEnabledHint': 'Controls whether transfers to S3 use TLS encryption. Turning it off uses an unencrypted connection.',
  'resources.storage.tlsVerifyHint': 'Turn this off only for private CA or self-signed certificates. Certificates will not be verified, which creates a man-in-the-middle risk.',
  'resources.storage.virtualHostedHint': 'Uses bucket.endpoint virtual-hosted addressing when enabled, or endpoint/bucket path-style addressing when disabled. Choose the mode supported by the S3 service.',
  'resources.storage.sourceFallbackDetail': 'Applies only when this profile is the default. When S3 has no cached model, enabling this allows the download to continue from the original model source; disabling it makes that download fail.',
  'resources.preheat.profile.deleteContent.default': 'Delete S3 profile "{name}"? After deletion, new downloads use the model source directly until another default S3 profile is configured.',
  'resources.preheat.profile.deleteContent.system': 'Delete S3 profile "{name}"? After deletion, the system stops using this S3 profile. If local S3 remains configured in the Server or Docker startup arguments, the profile will be created again after restart.',
  'resources.preheat.profile.deleteContent.systemDefault': 'Delete S3 profile "{name}"? After deletion, the system stops using this S3 profile and new downloads use the model source directly until another default S3 profile is configured. If local S3 remains configured in the Server or Docker startup arguments, the profile will be created again after restart.',
  'resources.storage.sync.noDefault': 'No active default S3 profile is available. Set a default profile first.',
  'resources.storage.sync.alreadyFromDefault': 'This model already comes from the default S3 model library and does not need to be synced again.',
  'resources.storage.sync.missingRevision': 'This legacy model has no trusted revision metadata. Download it again before syncing it to S3.',
  'resources.storage.revision.modelscopeFilelist': 'File-list fingerprint {fingerprint}',
  'resources.preheat.noReadyWorkers': 'No Ready workers are available for connectivity checking.',
  ...
  'resources.storage.syncBatch.description.single_model': 'Upload one existing model on the selected Ready worker to the target S3. It will not be distributed to other workers.',
  'resources.storage.syncBatch.description.selected_workers': 'Upload existing models on the selected Ready workers to the target S3. They will not be distributed to other workers.',
  'resources.storage.syncBatch.description.all_ready_workers': 'Upload existing models on all Ready workers to the target S3. They will not be distributed to other workers.',
  'resources.storage.syncBatch.selectWorker': 'Select Worker',
  'resources.storage.syncBatch.selectModel': 'Select Model',
  'resources.storage.syncBatch.noSyncableModels': 'This worker has no syncable Ready models.',
  ...
  'resources.storage.syncBatch.reason.model_file_not_ready': 'Model is not Ready',
  'resources.storage.syncBatch.reason.worker_not_ready': 'Worker is not Ready',
  'resources.storage.syncBatch.reason.duplicate_artifact_identity': 'The same model version is already planned from another Ready worker',
  'resources.storage.syncBatch.reason.active_task_exists': 'A sync task is already active',
  'resources.storage.syncBatch.reason.model_file_not_found': 'Model does not exist',
  'resources.storage.syncBatch.reason.no_ready_workers': 'No Ready workers are available',
  'resources.storage.deleteSync': 'Delete Sync Record',
  'resources.storage.deleteSyncConfirm': 'Delete Sync Record',
  'resources.storage.deleteSyncContent': 'Delete sync record #{id}? This cannot be undone.',
