export default {
  ...
  'resources.preheat.profile.maintenanceHint': 'Жизненный цикл не зависит от состояния подключения. Профили на обслуживании исключаются из выбора для новых загрузок, синхронизаций, прогрева, политик и расписаний.',
  'resources.preheat.profile.maintenanceAction': 'Перевести в обслуживание',
  'resources.preheat.profile.maintenanceActionHint': 'Исключает профиль из новых задач без удаления данных S3.',
  'resources.preheat.profile.restoreAction': 'Восстановить использование',
  'resources.preheat.profile.restoreActionHint': 'Снова разрешает использовать профиль для новых задач, но не назначает его по умолчанию.',
  'resources.preheat.profile.maintenanceConfirm': 'Перевести в режим обслуживания',
  'resources.preheat.profile.maintenanceContent': 'Перевести S3-профиль «{name}» в обслуживание? Он не будет доступен для выбора в загрузках, синхронизациях, прогреве, политиках и расписаниях. Данные S3 не удаляются, а существующие задачи продолжают работу с фиксированной конфигурацией.',
  'resources.preheat.profile.restoreConfirm': 'Восстановить S3-профиль',
  'resources.preheat.profile.restoreContent': 'Восстановить использование S3-профиля «{name}»? Он не будет автоматически назначен профилем по умолчанию.',
  'resources.preheat.profile.credentialUnavailableHint': 'При недоступном подключении обновите Access Key и Secret Key в ручном профиле. Это не переводит профиль в обслуживание автоматически.',
  'resources.preheat.profile.systemCredentialUnavailableHint': 'При недоступном подключении обновите учетные данные системной конфигурации через параметры запуска и перезапустите сервис. Это не переводит профиль в обслуживание автоматически.',
  'resources.preheat.profile.connectivityUnavailableHint': 'Сбой подключения может быть связан с DNS, TLS, сетью, аутентификацией или Bucket. Откройте сведения о подключении и проверьте этап сбоя; затем обновите учетные данные, TLS или адресацию ручного профиля. Профиль не переводится в обслуживание автоматически.',
  'resources.preheat.profile.systemConnectivityUnavailableHint': 'Сбой подключения может быть связан с DNS, TLS, сетью, аутентификацией или Bucket. Откройте сведения о подключении и проверьте этап сбоя; управляемые поля обновите через параметры запуска и перезапустите сервис, а доступные переключатели можно изменить на этой странице. Профиль не переводится в обслуживание автоматически.',
