...
const UsageTrends: React.FC<UsageTrendsProps> = ({
  ...
  const formatLocalHour = (hour: number) => {
    return dayjs(
      `${selectedDate}T${String(hour).padStart(2, '0')}:00:00Z`
    ).format('HH:mm');
  };
  ...
  const hourlySeries: HourlyLogRow[] = Array.from(
    { length: 24 },
    (_, hour) =>
                ...
                <Col span={24}>
                  <LineChart
                    ...
                    title={`${intl.formatMessage({
                      id: 'modelUsage.hourlyRequests'
                    })} / ${intl.formatMessage({ id: 'modelUsage.hourlyTokens' })}`}
                    xAxisData={xAxisData}
                    yAxisName={intl.formatMessage({
                      id: 'modelUsage.requestCount'
                    })}
                    secondaryYAxisName={intl.formatMessage({
                      id: 'modelUsage.totalTokens'
                    })}
                    legendData={[
                      intl.formatMessage({ id: 'modelUsage.requestCount' }),
                      intl.formatMessage({ id: 'modelUsage.totalTokens' })
                    ]}
                    seriesData={[
                      {
          ...
          },
          {
            ...
            children: (
              <Table
                ...
                pagination={false}
                rowClassName={(row) =>
                  row.date === selectedDate ? 'ant-table-row-selected' : ''
                }
                onRow={(row) => ({
